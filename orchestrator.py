...
def food_agent(
    ...
    if payload is not None:
        deal = payload.get("selected_deal", {})
        order = [{"name": deal.get("item_name", ""), "quantity": deal.get("portion", "1 serving")}]
...
def order_agent(payload: dict) -> dict:
    """
    TODO: wire to the real order_finalizer module when available as standalone.

    Drives the order_finalizer LangGraph and returns a summary dict.
    Returns: { "summary": "...", "promo_applied": bool, "final_price": "..." }
    """
    try:
        ...
        final_state = order_app.invoke({"payload": payload})
        ...
        # TODO: replace with real order_agent import when available as standalone module
        return {"summary": "", "promo_applied": False, "final_price": "0"}
...
You are a routing classifier for a food-ordering assistant called Food Pilot.

Classify the user message into exactly one of these modes:
  A — The user wants to ORDER food from a restaurant (must have or imply a location).
  B — The user is asking about a specific DISH (calories, ingredients, description, etc.).
  C — The user wants food that fits a CALORIE constraint AND wants to order (has a location or implies one).
  UNCLEAR — Not enough information; one clarifying question is needed.

Then extract:
  what  — the food entity, dish name, or calorie description (e.g. "pizza", "koshari", "under 400 kcal")
  where — the location string if mentioned, else empty string

Respond with EXACTLY this JSON (no extra text):
{{
  "mode": "<A|B|C|UNCLEAR>",
  "what":  "<extracted food entity or calorie description>",
  "where": "<location or empty string>",
  "clarification": "<one question to ask the user if mode is UNCLEAR, else empty string>"
}}
...
def _classify(message: str) -> dict:
    ...
    try:
        ...
        return eval(match.group()) if match else {}  # noqa: S307  (trusted LLM output)
    except Exception:
        return {"mode": "UNCLEAR", "what": "", "where": "", "clarification": "Could you clarify what you'd like?"}
...
def node_order(state: OrchestratorState) -> dict:
    print(f"\n[Orchestrator] ── order ──")
    payload = state.get("scout_payload", {})
    result = order_agent(payload=payload)
...
def build_orchestrator(checkpointer=None):
    ...
    g.add_edge("clarify", "classify")          # re-classify after clarification
    g.add_edge("calorie_filter", "scout")      # Mode C: filter → scout
    g.add_edge("scout", "enrich")              # Mode A: scout → enrich
    g.add_edge("enrich", "order")              # Mode A: enrich → order
    g.add_edge("food_inquiry", END)
    g.add_edge("order", END)

    # Mode C should skip enrich and go scout → order.
    # We achieve this by having the scout node detect Mode C and jump directly;
    # since LangGraph doesn't support per-edge conditions on the same source node
    # here, we use a conditional edge from scout instead.
    # Remove the unconditional scout→enrich edge and replace with conditional:
    # (done below — the add_edge above is overridden)
